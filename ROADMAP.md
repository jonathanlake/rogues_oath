...
- **STAMINA follow-ups (v0.24.0 experiment → GRADUATED v0.26.0 by Jeff's verdict 2026-07-26; the
  ...
  **NEW v0.28.0, two open items of its own:** (1) **the system's NAME** — Jeff asked for a better one than
  ...
  on top until then. Same shape for **`whiff_recovery_beats`** (v0.32.0, default **-1** = the whiff pays its
  FULL tail), which is the A/B switch for DESIGN §2.3.9 rather than a pending experiment — the default IS
  Jeff's answer (it was the old `whiff_pays_recovery` ON), the dial exists so he can re-compare, and v0.32.0
  turned the bool into a float so the middle Jon asked for is reachable too: `0` = pay none (the v0.26.0
  recovery-on-contact feel), `N` = pay N beats of the tail and hand the remainder back, capped at full.
...
- AFK / rest zones (Q5)
- **Ranged combat** → a CAPABILITY TRACK: the design + "complete when" live in **DESIGN §2.9**;
  stage status is tracked here — v1 shipped v0.17.0–v0.17.3; remaining: LoS-proper, monster archers,
  more weapons, gamepad aim, ranged backstab. One item stays parked here because it's not ranged-only:
...
**FUNCTION vs FEEL split (from M4a onward):** every milestone's **Done =** lists only
harness-checkable assertions (event traces, probe files, screenshots, assertion scripts —
what the `harness-verify` skill can demonstrate without a human). Anything that needs a
