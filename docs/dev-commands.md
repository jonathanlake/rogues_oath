...
| `/stamina` (alias `/mp`) | Toggle the **stamina experiment** (v0.24.0, renamed v0.24.1) live. OFF = byte-for-byte pre-experiment movement and AI (no pool mutation); ON = every pool reseeds to full. Host-authoritative (all reads are host-side). g-row dials: `regen_idle_beats` (10), `regen_interval_beats` (4), `exhausted_step_beats` (5 — the 0-stamina crawl speed). |
