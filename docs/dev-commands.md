...
| `/trait <name> [remove]` | **v0.45.0.** GRANT a trait to yourself, or take it back. Resolves through `passive_catalog` by display-name slug **or `.tres` filename** (`sneak_attack` and `backstab` both work — the file and the display name diverge on purpose). Sender-only, like `/class`. Refused if your CLASS already gives you that trait: granting a duplicate used to stack it and run its combat hook twice. Granted traits die with the player. |
| `/pa <trait> <field> <value\|reset>` | **v0.45.0.** Tune a trait live, like `/w` does weapons — the mutation lands on the shared `.tres`, so it reaches every wearer at once. Fields: `damage_multiplier`, `windup_beats_delta`, `recovery_beats_delta`. A trait's fields live on its own script, so naming one it does not have is REFUSED rather than silently ignored. |
