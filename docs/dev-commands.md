...
| `/mp` | Toggle the **movement-points experiment** (v0.24.0) live. OFF = byte-for-byte pre-experiment movement and AI (no pool mutation); ON = every pool reseeds to full. Host-authoritative (all reads are host-side). The regen dials are `/config` g-row fields: `regen_idle_beats` (default 10), `regen_interval_beats` (default 4). |
